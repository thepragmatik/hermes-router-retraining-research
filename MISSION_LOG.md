...
| 2026-09-08 | V1 baseline closed (plan 2026-09-08_200345-v1-baseline-gaps.md, no code changes): untracked state landed after pii_sweep PII_HITS=0 (gen-factory artifacts, shadow log 67 rows +1 probe, handoff, extended sweep tool); router_config normalized (enabled, 0.30 canonical; drift guard re-verified exit=2); shadow-log gaps G1-G6 documented in results/V1_BASELINE_GAPS.md (prompt_id hardcoded 0 at router_v1_cli.py:69; service never logs; no prompt hash; no outcome; volume health-only; config drift cosmetic). V1 router untouched. Branch + tag v1-baseline-2026-09-08 pushed; local==remote verified. Shadow fix (hash + outcome join) deferred to fresh prereg. |
