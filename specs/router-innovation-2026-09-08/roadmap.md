...
|---|---|---|---|---|---|---|
| 101 | Counterfactual Shadow Telemetry | randomized sentinels + known propensities + joinable outcomes | learning substrate; does not train the final router | none | qualified (`SHADOW_READY`): Stage 0 `STAGE0_PASS`; Stage-1 gates all PASS (`results/101/STAGE1_DATA_QUALITY.md`); Phase 2-5 merged (`70d46ca`) and **deployed to production** (launchd `com.rath.router-shadow-v1`, 127.0.0.1:8765; `results/101/DEPLOY_VERIFICATION.md`). Logs real decisions with sha256 join keys, no raw prompt text; exploration `disabled` (operator prereg required). No joined real outcomes yet — 102/108 real phases gated on accumulation | [`../101-counterfactual-shadow-telemetry/spec.md`](../101-counterfactual-shadow-telemetry/spec.md) |
